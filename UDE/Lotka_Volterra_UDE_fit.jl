# Introduction
#=
In this example, we will generate data from a Lotka-Volterra predator-prey model. 
Following this, we will construct a Universal Differential Equation (UDE) by replacing 
interaction terms with a neural network. This example demonstrates:
1. How to write code which integrates neural networks with ODEs in Julia.
2. How to optimize the neural network parameters and train the resulting UDE.
3. Such a framework can be used in various ecological and biological systems modeling.
=#
...
# Neural networks to learn the interaction terms
NN1 = Lux.Chain(Lux.Dense(2, 50, relu), Lux.Dense(50, 1))  # Prey interaction
NN2 = Lux.Chain(Lux.Dense(2, 50, relu), Lux.Dense(50, 1))  # Predator interaction
...
iter = 0
function callback(θ, l, pred; doplot=true, opt_method="Adam", iteration = 0)
...
# First optimization run using Adam
result_ude = Optimization.solve(optprob, OptimizationOptimisers.Adam(0.001), callback=(p, l) -> callback(p, l, predict_ude(p)), maxiters=1000)

# Further optimization with BFGS
using Optim
optprob2 = Optimization.OptimizationProblem(optf, result_ude.minimizer)
res_bfgs = Optimization.solve(optprob2, Optim.BFGS(initial_stepnorm=0.001), callback=(p, l) -> callback(p, l, predict_ude(p), opt_method="BFGS"), maxiters=2000)
...
sol_forecast = Array(solve(prob_forecast, Tsit5(), p=res_bfgs.minimizer, saveat=t_forecast))
...
# Discussion
# The model successfully learns the interaction terms in the Lotka-Volterra system, 
# and forecasts are made for future population dynamics.
